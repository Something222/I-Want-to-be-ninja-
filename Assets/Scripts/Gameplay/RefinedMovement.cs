...
public class RefinedMovement : MonoBehaviour
{
    ...
    #endregion shitdone
    //so gameplay is essentially done just need the fluff.
    //MENUS ARE IMPORTANT (STILL TO DO)
    ...
    //seeing the arms and legs animated would be nice but with the wonky jamp animation it dont look to great and i clip inside the model no way to just select hands
    ...
    //THE WRITE UP
    //replace the place holder loading screen (do it last cause it still works just has a random egg on it though)
    //if theres time refactor scripts cause they are messy
    #endregion shittodo
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown("Sprint") && !Sprinttoggle && !shifttoggle && z > 0 && !Isjumping)
        {
            Sprinttoggle = true;
            shifttoggle = true;
        }
        ...
        if (Input.GetButtonDown("Jump") && secondjump && !iswallrunning && !isWallClimbing)
        {

            JumpFormula(false);
            ////for the damn hookshot FUCK
            //ResetXVelocity();
            //ResetZVelocity();
            isjumping = true;
        }
        ...
        if (Input.GetButtonDown("Jump") && !isGrounded && fallingjump)
        {
            JumpFormula(false);
        }
        if (iswallrunning)
        {
            gravity = defaultgrav * .65f;
